...
def hpss(mag_specgrams, kernel_size=31, power=2.0, hard=False, mask_only=False):
    ...
    x_padded = F.pad(mag_specgrams, pad=pad, mode='reflect')

    _enhance_either_hpss(x_padded, out=perc, kernel_size=kernel_size[0], power=power, which='perc',
                         offset=kernel_size[1] // 2)
    _enhance_either_hpss(x_padded, out=harm, kernel_size=kernel_size[1], power=power, which='harm',
...
def pss_src(waveforms, kernel_size=31, power=2.0, hard=False):
    ...
    n_fft = 1024
    hop_length = 256
    complex_stfts = torch.stft(waveforms, n_fft=n_fft, hop_length=hop_length)
    mag_stfts = complex_stfts.pow(2).sum(-1).unsqueeze(1)  # add channel dim
    _, _, _, mask_perc = hpss(mag_stfts, kernel_size, power, hard, mask_only=True)
    mask_perc.squeeze_(1).unsqueeze_(3)  # remove channel, add last dim for complex
    percusive_waveforms = time_freq.istft(complex_stfts * mask_perc, hop_length=hop_length, length=waveforms.shape[1])
    return percusive_waveforms
